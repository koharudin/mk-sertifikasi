...
const ListTable = ({ sekolah_uuid }) => {
  ...
  const handleDelete = async (id: number, name: string) => {
    const result = await Swal.fire({
      title: 'Hapus Data?',
      text: `Student "${name}" akan dihapus.`,
      icon: 'warning',
    ...
    try {
      await api.delete(`/master/sekolah/${sekolah_uuid}/student/${id}`)
  ...
  const handleDetail = async (id: number) => {
    try {
      ...
      const response = await api.get(
        `/master/sekolah/${sekolah_uuid}/student/${id}`
      )
  ...
  const getStudent = async () => {
    try {
      ...
      const response = await api.get(`/master/sekolah/${sekolah_uuid}/student`, {
        params: {
  ...
  const columns = useMemo<ColumnDef<StudentType>[]>(
    () => [
...

      {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-2'>
            <Tooltip title='Data Wali Murid'>
              <IconButton color='secondary'>
                <i className='tabler-user-heart' />
              </IconButton></Tooltip>
